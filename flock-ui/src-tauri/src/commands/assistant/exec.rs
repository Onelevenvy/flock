...
) -> Result<(flock_agent::engine::AgentEngine, bool, bool)> {
...
pub async fn send_message_to_engine(
    ...
    let (workdir, assistant_id, extra_args) = {
        let s = state.lock().await;
        if execution_manager.is_task_running(&sid).await {
    ...
    let cancel_flag_for_guard = cancel_flag.clone();
    ...
    impl Drop for SessionCleanupGuard {
        fn drop(&mut self) {
            ...
            tokio::spawn(async move {
                ...
                execution_manager.unregister_task(&session_id).await;
    ...
    let join_handle = tokio::spawn(async move {
        let _guard = SessionCleanupGuard {
            state: state_clone,
            execution_manager: execution_manager_clone,
            session_id: sid_clone2,
            msg_id: msg_id_clone2,
            cancel_flag: cancel_flag_for_guard,
        ...
        if let Err(err) = run_result {
            if matches!(err, flock_agent::engine::AgentError::UserAborted) || cancel_flag_for_spawn.load(Ordering::SeqCst) {
    ...
    if cancel_flag_for_check.load(Ordering::SeqCst) {
        join_handle.abort();
    } else {
        execution_manager.register_task(sid, join_handle, cancel_flag, is_running).await;
    }
...
pub async fn stop_agent(
    ...
    let sid = session_id.unwrap_or_else(|| "default".to_string());
    execution_manager.cancel_task(&sid).await;
    Ok(())
}
