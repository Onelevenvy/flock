...
def upgrade():
    ...
    op.create_table('toolprovider',
    sa.Column('provider_name', sqlmodel.sql.sqltypes.AutoString(length=64), nullable=False),
    sa.Column('mcp_endpoint_url', sqlmodel.sql.sqltypes.AutoString(), nullable=True),
    sa.Column('mcp_server_id', sqlmodel.sql.sqltypes.AutoString(), nullable=True),
    sa.Column('mcp_connection_type', sqlmodel.sql.sqltypes.AutoString(), nullable=True),
    ...
    sa.Column('tool_type', sa.Enum('BUILTIN', 'API', 'MCP', name='tooltype'), nullable=False),
    ...
    sa.Column('tool_definition', postgresql.JSONB(astext_type=sa.Text()), nullable=True),
    sa.Column('input_parameters', postgresql.JSONB(astext_type=sa.Text()), nullable=True),
    sa.Column('provider_id', sa.Integer(), nullable=False),
    sa.Column('is_online', sa.Boolean(), nullable=True),
    sa.Column('id', sa.Integer(), nullable=False),
    sa.ForeignKeyConstraint(['provider_id'], ['toolprovider.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    op.add_column('memberskillslink', sa.Column('tool_id', sa.Integer(), nullable=False))
    op.drop_constraint(op.f('memberskillslink_skill_id_fkey'), 'memberskillslink', type_='foreignkey')
    op.create_foreign_key(None, 'memberskillslink', 'tool', ['tool_id'], ['id'])
    op.drop_column('memberskillslink', 'skill_id')
    op.add_column('modelprovider', sa.Column('is_available', sa.Boolean(), nullable=True))
...
def downgrade():
    ...
    op.add_column('memberskillslink', sa.Column('skill_id', sa.INTEGER(), autoincrement=False, nullable=False))
    op.drop_constraint(None, 'memberskillslink', type_='foreignkey')
    op.create_foreign_key(op.f('memberskillslink_skill_id_fkey'), 'memberskillslink', 'skill', ['skill_id'], ['id'])
