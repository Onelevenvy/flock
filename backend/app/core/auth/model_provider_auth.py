...
from app.core.model_providers.manager import model_provider_manager
...
async def authenticate_provider(session: Session, provider_id: int) -> Tuple[bool, str]:
    ...
    try:
        # 直接通过manager获取鉴权模型名
        credentials_model_name = model_provider_manager.get_credentials_model_name(provider.provider_name)
        if not credentials_model_name:
            logger.warning(f"提供商 {provider.provider_name} 未指定鉴权模型，将使用第一个模型")
            test_model = models[0]
        else:
            # 查找匹配的模型
            test_model = next((m for m in models if m.ai_model_name == credentials_model_name), None)
            if not test_model:
                logger.warning(f"找不到指定的鉴权模型 {credentials_model_name}，将使用第一个模型")
                test_model = models[0]
