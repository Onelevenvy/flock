...
/// - Falls back to `skills` inside the `resources` directory
pub fn workspace_skills_dir() -> PathBuf {
    // 1. 优先检查默认安装目录（比如开发环境下的 <exe_dir>/resources/skills，或通过环境变量指定的路径）
    ...
    if let Ok(exe_path) = std::env::current_exe() {
        if let Some(exe_dir) = exe_path.parent() {
            ...
            // 例如：<exe_dir>/_up_/_up_/resources/skills
            let win_linux_up = exe_dir.join("_up_").join("_up_").join("resources").join("skills");
            if win_linux_up.is_dir() {
                return win_linux_up;
            }

            // B. Windows/Linux: <exe_dir>/resources/_up_/_up_/resources/skills
            let win_linux_resource_up = exe_dir.join("resources").join("_up_").join("_up_").join("resources").join("skills");
            ...
            // D. macOS: <exe_dir>/../Resources/_up_/_up_/resources/skills
            let macos_resource_up = exe_dir.join("..").join("Resources").join("_up_").join("_up_").join("resources").join("skills");
